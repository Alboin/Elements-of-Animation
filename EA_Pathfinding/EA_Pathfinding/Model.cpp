...
Model::Model()
{
	m_calCoreModel = new CalCoreModel("skeleton");
...
bool Model::onInit(const std::string& strFilename)
{
	...
	for (materialId = 0; materialId < m_calCoreModel->getCoreMaterialCount(); materialId++)
	{
		...
		for (mapId = 0; mapId < pCoreMaterial->getMapCount(); mapId++)
		{
			...
			//GLuint textureId; TEMP
			//textureId = loadTexture(strPath + strFilename); TEMP

			// store the opengl texture id in the user data of the map
			//pCoreMaterial->setMapUserData(mapId, (Cal::UserData)textureId); TEMP
		}
	}
